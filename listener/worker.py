...
def handle_job(job: dict) -> bool:
    ...
    # A task job whose task was deleted or completed after it was queued must not
    # wake the device (HTTP/voice edits cancel the job, this is the safety net).
    task_id = data.get("task_id")
    if job["kind"] == "task" and task_id:
        rows = execute_query("SELECT status FROM tasks WHERE task_id = %s", (task_id,))
        if not rows:
            log.info("job %s: task %s no longer exists, dropping", job_id, task_id)
            return True
        if rows[0].get("status") not in (None, "pending"):
            log.info("job %s: task %s is %s, dropping", job_id, task_id, rows[0].get("status"))
            return True
