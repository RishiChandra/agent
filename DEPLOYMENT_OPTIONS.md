...
_Update 2026-09-08: added the orchestrator-TTS decision — improve voice naturalness with a **self-hosted** upgrade (a
better Piper voice now, Kokoro-82M as the next step), **not** cloud TTS (§1.7, §5 item 9)._
...
6. **Orchestrator voice quality is a self-hosted upgrade, decoupled from the cloud choice.** The robotic voice is the
   `amy-medium` Piper model, not TTS in general. Fix it by swapping to a higher-quality Piper voice now and adopting
   Kokoro-82M as a local backend behind the existing seam — keeping TTS at $0 marginal, on-CPU and portable. Cloud
   TTS is deliberately not adopted: it is the only path to truly human audio but would add tens of thousands to
   ~$180k a month at 1M users and a provider tie (§1.7). **Decided 2026-09-08.**
...
- Kairos minutes are the product decision that sets the bill; every 10 % of minutes on Kairos ≈ $0.17 per
  user-month.
- TTS stays self-hosted (§1.7): the routing-turn CPU line above already covers Piper/Kokoro, so no per-character
  cloud-TTS line is added. A cloud voice at 1M users would add roughly $25k (basic tier, ~$4/1M chars) to $180k+
  (premium, ~$30/1M chars) a month on top — on the order of the whole routing-LLM bill — which is why the
  naturalness fix is kept local.
...
### 1.7 Orchestrator voice quality (TTS) (new criterion, decided 2026-09-08)

The orchestrator speaks through **Piper** (`en_US-amy-medium`, local neural ONNX, 24 kHz) behind one seam —
`app/developer_ws/tts.py:synthesize_speech_pcm24_stream`, consumed by `PiperTTSProcessor`. The known complaint is
that the voice sounds robotic; that is the `amy-medium` voice tier (≈ 2.4 / 5 on naturalness against ≈ 4.2 for real
human audio), not TTS in general. The ladder of alternatives:

| Tier | Option | Runs on | Naturalness | Marginal cost | Integration |
|---|---|---|---|---|---|
| Free win | Better Piper voice (`en_US-lessac-high`) | same CPU | modest bump, still Piper character | $0 | `PIPER_MODEL_PATH` env var, no code |
| **Best local** | **Kokoro-82M** (StyleTTS2/ISTFTNet, Apache-2.0, 54 presets, no cloning) | CPU at ~real-time | large jump, near-cloud for its size | $0 | new backend behind the seam |
| Top local | Chatterbox 0.5B (MIT) | wants a GPU | frontier-ish | $0 licence | new backend + GPU instance |
| Cloud | Chirp 3 HD / Gemini TTS / ElevenLabs / Cartesia | vendor API | human | $4–50 / 1M chars | swap for the vendor's Pipecat service |

**Decision:** improve naturalness **self-hosted**, in two steps, and do **not** move TTS to a cloud vendor:

1. Swap `amy-medium` → `en_US-lessac-high` now (env var only; test whether it is good enough).
2. If it is not, add **Kokoro-82M** as a config-selectable backend behind `synthesize_speech_pcm24_stream`.

**Why this and not cloud TTS:** Kokoro keeps TTS at **$0 marginal, on-CPU and portable** — it changes nothing in the
§1.1 cost model, the §1.5 implementation estimates, or the §1.6 exit story, and it stays the natural fit for OCI (no
first-party TTS there) and for the portable-container design. A cloud voice is the only route to truly human audio,
but it reintroduces exactly the two things the rest of this document works to avoid: a per-use bill (tens of
thousands to $180k+/month at 1M users, §1.1) and a provider tie — it would have to follow the cloud choice, like the
Gemini LLM in §2.2 and §3. Chatterbox is the best self-hosted quality but forces a GPU instance, which changes the
routing-tier SKU; deferred unless lessac/Kokoro prove insufficient.

**Interaction with the hosting choice: none, by design.** Because the upgrade stays behind the existing seam, TTS is
provider-independent and this can ship now, on Azure, ahead of and regardless of where the orchestrator lands.
Revisit only if the product later wants human-grade voice (then match the vendor to the chosen cloud) or turns the
orchestrator into an assistant-first, conversational agent (then Gemini Live, at the cost model in §1.1).
...
9. **Orchestrator TTS naturalness (self-hosted, §1.7):** swap `en_US-amy-medium` → `en_US-lessac-high` now (env var
   `PIPER_MODEL_PATH`, no code change); make TTS a config-selected backend behind
   `app/developer_ws/tts.py:synthesize_speech_pcm24_stream` and add Kokoro-82M as the higher-naturalness local
   option. No cloud TTS — this keeps the $0-marginal, portable economics intact.
...
- Whether `en_US-lessac-high` is natural enough, or Kokoro-82M is warranted (§1.7).
...
- Piper naturalness and en_US voices (amy vs lessac): https://www.cekura.ai/discover/piper-tts , https://quick-tts.com/blog/piper-voices-ranked.html
- Kokoro-82M and self-hosted TTS comparison (Kokoro / Chatterbox / Piper): https://kompozy.io/reviews/kokoro-tts , https://www.sevenlabs.site/blogs/best-self-hosted-tts-models-2026
- Cloud TTS list pricing for the avoided-cost estimate (Chirp 3 HD, Azure Neural, Standard): https://cloud.google.com/text-to-speech/pricing
