...
_Update 2026-09-08: the orchestrator TTS voice is being upgraded within the self-hosted model (better Piper voice →
Kokoro-82M, not cloud TTS); see §2.1, the §4 cross-cutting notes, and DEPLOYMENT_OPTIONS.md §1.7._
...
| Persistent assets | `/home/data/vosk-model-small-en-us-0.15/` (~68 MB) and `/home/data/piper_voices/en_US-amy-medium.onnx` (+`.json`, ~63 MB), uploaded once via Kudu; paths passed as `VOSK_MODEL_PATH` / `PIPER_MODEL_PATH`. Both servers preload them at startup. The Piper voice is a drop-in `.onnx`; the planned naturalness upgrade (DEPLOYMENT_OPTIONS.md §1.7) swaps it for `en_US-lessac-high`, with Kokoro-82M as a higher-quality local backend — both stay on-disk and CPU-bound. | `README.md` "One-time bootstrap"; `app/main.py` lifespan |
...
- Orchestrator TTS will be improved for naturalness but stays **self-hosted** (a higher-quality Piper voice, then
  ...
  dependency with no new external service and no per-use cost — deliberately not moved to a cloud TTS
