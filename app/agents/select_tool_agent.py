        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "   - If the user asks 'What tasks do I have', 'Show me my tasks', 'When do I have X', 'Check my tasks', etc. → ALWAYS select 'get_tasks_tool'\n"
                    "   - NEVER select 'create_tasks_tool' for read-only queries, even if you see task data in the chat history\n"
                    "   - After 'get_tasks_tool' returns results, ALWAYS select 'generate_response_tool' next (do NOT select 'create_tasks_tool')\n"
