...
flows between the user and your service through the orchestrator. Uplink is
always the user's mic audio. Downlink is either audio your service synthesizes
itself, **or** plain text the orchestrator synthesizes for you via its built-in
TTS — your choice per frame.
...
8. Log every connection lifecycle event clearly (startup, tunnel ready,
   ...
   shutdown). Operators must be able to triage misconnects from logs alone.
